...
function ListingsInner({ forcedView }: { forcedView: 'community' | 'market' }) {
  ...
  // The Board/Marketplace split (each is its own route now) pins the view
  // per page; the old auth-dependent default and tab state are gone. The
...
export default function BoardHub({ forcedView }: { forcedView: 'community' | 'market' }) {
  return (
    <Suspense>
      <ListingsInner forcedView={forcedView} />
    </Suspense>
  )
}
