...
- **Schema changes: any migration with an SQL body (backfill, data move, rename, drop) ships via `prisma migrate deploy` — never hand-run `prisma db push` against prod.** Nuance that looks like a contradiction but isn't: `deploy.sh` itself runs `db push` on the server before the restart. That built-in push is the sanctioned *additive-only* sync (new nullable columns, new indexes) — it refuses destructive changes (exit 90, deploy warns loudly) and runs behind the pending-migrations gate. The ban is on pushing BY HAND and on using push for anything that needs SQL to run: `db push` syncs columns but executes no migration body and records nothing in `_prisma_migrations`. On 2026-08-16 that put three columns into production with no history row and silently skipped a backfill — which was the entire point of the migration that owned it, so testimonials sat unassigned and would have leaked Istanbul's quotes onto every other city's page. It also leaves the history so inconsistent that the next real `migrate deploy` fails trying to re-add an existing column. If columns already exist out-of-band, reconcile with `prisma migrate resolve --applied <name>` (records history without re-running SQL — so run any backfill by hand *first*).
