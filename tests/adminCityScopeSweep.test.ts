...
vi.mock('@/lib/notify',  () => ({ createNotification: vi.fn(async () => {}), notifyNewArticle: vi.fn(async () => {}), recipientSkipReason: vi.fn(() => null) }))
vi.mock('@/lib/email',   () => ({ sendBroadcastEmail: vi.fn(), sendLoginNudgeEmail: vi.fn(), recordEmailFailure: vi.fn() }))
vi.mock('@/lib/rateLimit', () => ({ rateLimit: vi.fn(async () => true), claimOnce: vi.fn(async () => true), releaseClaim: vi.fn(async () => {}), rateLimitRemaining: vi.fn(async () => 5), getIp: () => '127.0.0.1' }))
