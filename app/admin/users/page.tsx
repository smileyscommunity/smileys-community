...
function AdminUsersPageInner() {
  ...
  async function toggleHidden(u: DBUser) {
    ...
    const res = await fetch(`/app/api/admin/users/${u.id}`, {
      method: 'PATCH', credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ hiddenFromMembers: hiding }),
    })
    if (res.ok) {
      setUsers(prev => prev.map(x => x.id === u.id ? { ...x, hiddenFromMembers: hiding } : x))
      toast.success(hiding ? `${u.name} hidden from members list` : `${u.name} visible in members list again`)
    } else {
      const d = await res.json().catch(() => ({}))
          ...
          {visible.map(u => {
            ...
            const userActions = (
              <div className="flex gap-1.5 items-center">
                ...
                {isAdmin && (
                  <button onClick={() => toggleHidden(u)}
                    className={`p-2 rounded-lg transition-colors ${u.hiddenFromMembers ? 'text-violet-400 hover:bg-violet-500/10' : 'text-zinc-500 hover:bg-zinc-700/50'}`}
                    title={u.hiddenFromMembers ? 'Show in members list' : 'Hide from members list'}>
                    {u.hiddenFromMembers ? (
