...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const event = await prisma.event.findUnique({
      where: { id },
      select: { id: true, title: true, emoji: true, date: true, location: true, cityId: true, price: true, memberPrice: true, payTo: true, ticketUrl: true, paymentContact: true },
    })
    ...
    await Promise.all(attendees.map(async (a) => {
      ...
      await Promise.allSettled([
        sendEventReminderEmail(user.id, user.email, user.name, event.title, emoji, eventDate, eventLocation, event.id, { free: noShowPolicyApplies(event) })
          .then(() => { emailed++ }),
        createNotification(
          user.id,
          'reminder_24h',
