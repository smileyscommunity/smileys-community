...
// /students — the city-agnostic entry the landing page links to. The hub
// itself is per city (/[city]/students); this sends the reader to theirs:
...
export default async function StudentsEntry({ searchParams }: { searchParams?: Promise<CitySearch> }) {
  const { city } = await resolveCityForPage(searchParams)
  const pub = await getPublicCity(city.slug)
  redirect(`/${pub?.status === CITY_STATUS.Live ? city.slug : DEFAULT_CITY_SLUG}/students`)
}
