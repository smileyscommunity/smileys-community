    ...
    <div className="min-h-screen bg-warm pb-16">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <Link href={`/events/${id}`} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors">
        ...
        <div className="bg-white rounded-2xl shadow-card overflow-hidden">
          {event.coverImage ? (
            ...
            <div className="bg-gradient-to-br from-amber-100 to-orange-100 p-8 text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">{formatDate(event.date)} · {event.neighborhood}</p>
            </div>
          )}
              ...
              {(checkinCount > 0 ? checkedIn : attendees).map(a => {
                ...
                return (
                  <Link key={a.userId} href={`/members/${a.userId}`}
                    ...
                    <span className="text-xs text-gray-500 text-center max-w-[56px] truncate">{a.user.name.split(' ')[0]}</span>
                  </Link>
                )
              })}
            </div>
