...
export default function DetailClient({
  business, initialIsSaved, initialMyReviewId, isLoggedIn, currentUserId, ownerEditPayload,
}: Props) {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 mt-5 pt-4 border-t border-gray-100">
        ...
        {/* Claim CTA — only visible when there's no verified owner yet.
            Anonymous viewers get bounced to login. */}
        {!business.hasClaimedOwner && (
          isLoggedIn ? (
            // Reuse the inline trigger pattern from the grid card by
            // opening the reviews drawer's claim sibling… but cleaner
            // here is just a link back to /directory where the chip
            // lives. Keep the surface tight on detail.
            <Link
              href="/directory"
            ...
            <Link
              href={`/login?next=${encodeURIComponent(`/directory/${business.id}`)}`}
              className="text-xs font-semibold text-gray-500 hover:text-amber-700 hover:underline"
            >
              Is this your business?
            </Link>
