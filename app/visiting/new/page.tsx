...
// Anonymous-friendly: visitors can post before they sign up. Turnstile is
// shown to non-members only (members are already past the human-check at
// login). Rate limit (3/day/IP) lives in the API.
...
export default function NewVisitingPage() {
  ...
  // Members: prefill name + skip email (we have it on file via session).
  useEffect(() => {
    if (isLoggedIn && user.name && !name) setName(user.name)
  }, [isLoggedIn, user.name, name])

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!isLoggedIn && !token) { setError('Please complete the human check'); return }
    ...
    try {
      const res = await fetch('/app/api/visitors', {
        ...
        body: JSON.stringify({
          name, email: email || undefined, fromCity: fromCity || undefined,
          intro, startsOn, endsOn, neighborhood: neighborhood || undefined,
          contact: contact || undefined, _cf: token,
        }),
      })
    ...
    <div className="min-h-screen bg-warm pb-16">
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-8 pb-6">
          ...
          <p className="text-base text-gray-500 mt-1">Members in Istanbul will see this and can reach out.</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-8">
        <form onSubmit={handleSubmit} className="space-y-5 pb-8">
          ...
          {!isLoggedIn && (
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                Email <span className="text-gray-400 font-normal">(only shown to logged-in members)</span>
              </label>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">
              WhatsApp / Instagram <span className="text-gray-400 font-normal">(optional, members-only)</span>
            </label>
