...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const where = (viewer ? [hangout.location, hangout.neighborhood] : [hangout.neighborhood]).filter(Boolean).join(' · ')
  const title = `${hangout.title} · Smileys hangout`
  const desc  = hangout.status === 'cancelled'
    ? 'This hangout was cancelled.'
    : `${where ? `📍 ${where} · ` : ''}🕒 ${when} — join on Smileys.`
...
export default async function HangoutPermalinkPage({ params }: PageProps) {
  ...
  const hangout = await prisma.hangout.findUnique({
    where: { id },
    include: {
      user: { select: { id: true, name: true, color: true, profilePhoto: true, goodHangouts: true, neighborhood: true, status: true } },
      joins: {
  ...
  if (!hangout) notFound()
  // A block in either direction closes this page like it closes the feed and
  // the join: the whole point of a block is that the blocked can't read the
  // blocker's live location and time. Same 404 as a missing hangout.
  if (hangout.user.id !== session.id) {
    const blocked = await prisma.memberBlock.findFirst({
      where: { OR: [{ blockerId: hangout.user.id, blockedId: session.id }, { blockerId: session.id, blockedId: hangout.user.id }] },
      select: { id: true },
    })
    if (blocked) notFound()
  }
  ...
  // A banned or suspended host can't hear a join: not joinable.
  const isJoinable = hangout.status === 'active' && hangout.endsAt >= new Date() && hangout.user.status === 'approved'
  // maxPeople counts the host, so joiners are capped at maxPeople - 1.
  const isFull     = !!hangout.maxPeople && hangout.joins.length >= hangout.maxPeople - 1
  // Same authority the DELETE endpoint enforces: host, or staff of THIS
  // city. Only offered while there is still something to call off.
  const canCancel  = isJoinable && (isOwner || isAdmin(session) || canActInCity(session, hangout.cityId))
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-6 pb-6 space-y-4">
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="text-sm text-gray-700 space-y-1">
              ...
              <p>🕒 <span className="font-medium">
                {hangout.startsAt.toLocaleString('en-GB', { timeZone: tz, weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' })}
                {' – '}
        ...
        {isOwner ? (
          <p className="text-center text-xs font-semibold text-gray-500">
            You're hosting this hangout{isJoinable && <> · <Link href="/hangouts" className="text-amber-600 hover:underline">Edit it in the feed</Link></>}
          </p>
        ) : isJoinable && isFull && !joinedByMe ? (
          <button disabled className="w-full text-sm font-bold px-4 py-3 rounded-2xl bg-gray-100 text-gray-500 cursor-not-allowed">
            This hangout is full ({hangout.maxPeople} people)
          </button>
        ) : isJoinable ? (
                  ...
                  <Link key={j.user.id} href={`/members/${j.user.id}`} className="flex items-center gap-2 text-xs">
                    {av
                      ...
                      : <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold"
                          style={{ backgroundColor: j.user.color }}>{getInitials(j.user.name)}</div>}
