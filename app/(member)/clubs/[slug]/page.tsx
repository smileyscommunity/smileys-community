    ...
    <div className="min-h-screen bg-warm pb-20 md:pb-0">
      ...
      <div className="relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className={`${club.coverImage ? '-mt-10 pb-6' : 'py-6'}`}>
            <div className={`inline-flex items-center gap-4 ${club.coverImage ? 'bg-white/95 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-sm' : ''}`}>
              <div className="w-16 h-16 rounded-2xl bg-white shadow-card flex items-center justify-center text-3xl shrink-0 ring-2 ring-white">
                {club.emoji}
              </div>
              <div>
                ...
                <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight mt-0.5 mb-1.5 text-gray-900">
                  {club.name}
                </h1>
      ...
      {/* Mobile join CTA — shown right below hero so it's always in view without scrolling */}
      {membershipStatus !== 'approved' && (
        <div className="lg:hidden max-w-7xl mx-auto px-4 sm:px-6 pt-5">
          <ClubJoinWidget
      ...
      {/* Mobile-only chat links — the same WhatsApp / Instagram CTAs live
          in the desktop sidebar's "Club info" card, but on mobile that
          card stacks at the very bottom (single-column layout) so members
          had to scroll past every section to find the group chat. Surface
          them here under the hero. Desktop hides this strip (lg:hidden);
          only renders when there's at least one link to show and the
          viewer is an approved member (non-members get the join CTA
          above, not the chat link). Instagram is always public so it
          renders for any approved member with a link. */}
      {membershipStatus === 'approved' && (club.whatsappUrl || club.instagramUrl) && (
        <div className="lg:hidden max-w-7xl mx-auto px-4 sm:px-6 pt-3 flex flex-wrap gap-2">
          {club.whatsappUrl && (
            <a
              href={club.whatsappUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 min-w-[160px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-green-50 hover:bg-green-100 transition-colors text-sm font-semibold text-green-700"
            >
          ...
          {club.instagramUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 min-w-[160px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-pink-50 hover:bg-pink-100 transition-colors text-sm font-semibold text-pink-700"
            >
              <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            {((communitySettings.communityRules?.length ?? 0) > 0 || club.rules) && (
              <details id="club-rules" className="bg-amber-50 border border-amber-100 rounded-2xl mb-8 group scroll-mt-24">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-card p-6">
              ...
              {/* Desktop-only — the same club WA/IG block renders in
                  the mobile strip under the hero (see lg:hidden block
                  above). On a single-column layout the sidebar stacks
                  at page bottom, so rendering both at < lg shipped the
                  same Instagram twice. */}
              {(club.whatsappUrl || club.instagramUrl) && (
                <div className="hidden lg:block mt-5 pt-4 border-t border-gray-100 space-y-2">
                  {club.whatsappUrl && (
                    canSeeMemberContent ? (
                    <a
                      href={club.whatsappUrl}
              ...
              {/* Community-wide "Follow Smileys" footer block. Shown
                  with a distinct label so it doesn't compete with the
                  per-club links above. Dedupe per-channel: when a club
                  has reused the community URL for its own WhatsApp /
                  Instagram, the matching community icon is suppressed
                  (otherwise the same link rendered twice with two
                  different labels looked like a bug). */}
              {((communityIg && !sameSocialUrl(communityIg, club.instagramUrl)) || (communityWa && !sameSocialUrl(communityWa, club.whatsappUrl))) && (
                <div className="mt-5 pt-4 border-t border-gray-100">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                    Smileys Community
                  </p>
                  <div className="flex gap-2">
                    {communityWa && !sameSocialUrl(communityWa, club.whatsappUrl) && (
                      <a
                        href={communityWa}
                        target="_blank"
                        rel="noopener noreferrer"
                        title="Smileys Community WhatsApp"
                        className="flex items-center justify-center w-9 h-9 rounded-xl bg-gray-100 hover:bg-green-50 text-gray-600 hover:text-green-600 transition-colors"
                      >
                    ...
                    {communityIg && !sameSocialUrl(communityIg, club.instagramUrl) && (
                      <a
                        href={communityIg}
                        target="_blank"
                        rel="noopener noreferrer"
                        title="Smileys on Instagram"
                        className="flex items-center justify-center w-9 h-9 rounded-xl bg-gray-100 hover:bg-pink-50 text-gray-600 hover:text-pink-600 transition-colors"
                      >
                        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                        </svg>
