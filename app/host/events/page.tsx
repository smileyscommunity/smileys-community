...
function StatusMenu({ e, onStatusChange }: { e: Event; onStatusChange: (id: string, status: string) => void }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} title="Change status"
        className="p-2 rounded-lg bg-zinc-800 text-zinc-400 hover:bg-zinc-700 border border-zinc-700 transition-colors">
...
function EventRow({ e, onDuplicate, onStatusChange }: { e: Event; onDuplicate: () => void; onStatusChange: (id: string, status: string) => void }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center gap-2 pt-1 border-t border-zinc-800">
        ...
        <StatusMenu e={e} onStatusChange={onStatusChange} />
      </div>
    </div>
  )
}
...
export default function HostEventsPage() {
  ...
  async function handleStatusChange(id: string, status: string) {
    const labels: Record<string, string> = { cancelled: 'Cancel', postponed: 'Postpone', archived: 'Archive', published: 'Publish' }
        ...
        {([
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-5 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${
              tab === t.key ? 'border-amber-500 text-amber-400' : 'border-transparent text-zinc-500 hover:text-zinc-200'
            }`}>
            {t.label}
            {t.count > 0 && (
              <span className={`ml-1.5 text-xs font-bold px-1.5 py-0.5 rounded-full ${
                tab === t.key ? 'bg-amber-500/20 text-amber-400' : 'bg-zinc-800 text-zinc-500'
              }`}>{t.count}</span>
            )}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-12 text-center">
          ...
          <div className="text-zinc-500 text-sm mb-6">
            {tab === 'upcoming' ? 'Create your first event to get started.' : 'Past events will appear here.'}
          </div>
          {tab === 'upcoming' && (
        ...
        <div className={`space-y-3 ${tab === 'past' ? 'opacity-75' : ''}`}>
          {displayed.map(e => <EventRow key={e.id} e={e} onDuplicate={() => handleDuplicate(e.id)} onStatusChange={handleStatusChange} />)}
        </div>
      )}
    </div>
  )
