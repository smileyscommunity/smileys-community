#!/bin/bash
# Cron-driven sweeper deleting orphaned applicant photos from the uploads
# applications/ folder (see app/api/cron/sweep-orphan-uploads/route.ts).
# Designed to run nightly from system crontab on the prod box.
#
...
#
# Dry run by hand (lists what it would delete, deletes nothing):
#   SMILEYS_SWEEP_ENDPOINT='http://localhost:3000/app/api/cron/sweep-orphan-uploads?dryRun=1' \
#     scripts/sweep-orphan-uploads.sh
...
exec 9>"/tmp/sweep-orphan-uploads.lock"
flock -n 9 || { echo "$(date -u +%FT%TZ) skipped: previous run still active"; exit 0; }

ENV_FILE="${SMILEYS_ENV_FILE:-/root/smileys-community/.env}"
ENDPOINT="${SMILEYS_SWEEP_ENDPOINT:-http://localhost:3000/app/api/cron/sweep-orphan-uploads}"
...
OUT="/tmp/sweep-orphan-uploads.out"
