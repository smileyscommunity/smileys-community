#!/bin/bash
# Cron-driven sweeper for pay-in-advance payment reminders (see
# app/api/cron/sweep-payment-reminders/route.ts). Designed to run hourly
...
exec 9>"/tmp/sweep-payment-reminders.lock"
flock -n 9 || { echo "$(date -u +%FT%TZ) skipped: previous run still active"; exit 0; }

ENV_FILE="${SMILEYS_ENV_FILE:-/root/smileys-community/.env}"
ENDPOINT="${SMILEYS_SWEEP_ENDPOINT:-http://localhost:3000/app/api/cron/sweep-payment-reminders}"
...
OUT="/tmp/sweep-payment-reminders.out"
