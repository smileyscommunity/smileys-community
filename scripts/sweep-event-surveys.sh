#!/bin/bash
# Cron-driven sweeper that dispatches post-event safety surveys (see
# app/api/cron/sweep-event-surveys/route.ts). Runs hourly from system
...
exec 9>"/tmp/sweep-event-surveys.lock"
flock -n 9 || { echo "$(date -u +%FT%TZ) skipped: previous run still active"; exit 0; }

ENV_FILE="${SMILEYS_ENV_FILE:-/root/smileys-community/.env}"
ENDPOINT="${SMILEYS_SURVEY_SWEEP_ENDPOINT:-http://localhost:3000/app/api/cron/sweep-event-surveys}"
...
OUT="/tmp/sweep-event-surveys.out"
